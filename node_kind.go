...
const (
	bad NodeKind = iota // is used only inside a C2V to mark unknown or corrupted node.
	access_spec_decl
...
// Use this style when defining a new record.
var str_to_node_kind_map = map[string]NodeKind{
	"BAD":                                    bad,
...
var node_kind_to_str_map = reverse_str_to_node_kind_map()
...
func convert_str_into_node_kind(value string) NodeKind {
	return str_to_node_kind_map[value]
}
...
func (kind NodeKind) str() string {
	return node_kind_to_str_map[kind]
}

func reverse_str_to_node_kind_map() map[NodeKind]string {
	reversed_map := map[NodeKind]string{}

	for key, value := range str_to_node_kind_map {
